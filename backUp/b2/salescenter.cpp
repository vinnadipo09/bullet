#include "salescenter.h"
#include "ui_salescenter.h"

SalesCenter::SalesCenter(QWidget *parent, loggedUser &currentLoggedInUser) :
    QDialog(parent),
    ui(new Ui::SalesCenter)
{
    ui->setupUi(this);
//    this->showMaximized();
...
void SalesCenter::closeEvent(QCloseEvent *event)
{
    QMessageBox msgBox;
...
void SalesCenter::loadItemsFromDbToCompleter()
{
...
void SalesCenter::scannedProductManagement(QString &barcode, int &currentProductId, int &stockAvailable)
{
...
void SalesCenter::modifyProductInRowCreated(int &rowAffected, int &quantityValue)
{
...
void SalesCenter::createRowsToAddProductPurchased(int &quantityValue)
{
...
void SalesCenter::addProductInRowCreated(int &rowCreated, int &quantityPurchased)
{
...
void SalesCenter::getScannedProductFromDB(QString barcodeScanned)
{
...
void SalesCenter::goToPaymentAndCompleteSale()
{
...
void SalesCenter::grabBarcodeFromEntry()
{
    uniqueID = ui->le_barcodeEntry->text();
    getScannedProductFromDB(uniqueID);
}

void SalesCenter::grabBarcodeFromCompleter(QString &currentProduct)
{
    if(salesConnection->conn_open()){
        QSqlQuery query(QSqlDatabase::database("MyConnect"));
        query.prepare(QString("SELECT products.product_id, products.productName, products.productBarcode, products.productMeasurement, products.productWSPrice"
...
void SalesCenter::modifyProductsInTableReduction(int &rowAffected, int &quantityValue)
{
    int columnsToModifyProducts = 9;
    for(int i =0; i<columnsToModifyProducts; i++){
        if(i==5){
...
void SalesCenter::deleteProductFromCart(int &rowAffected, int &quantityValue)
{
    int columnsToModifyProducts = 9;
    for(int i =0; i<columnsToModifyProducts; i++){
        if(i==5){
...
void SalesCenter::loadCustomersToCompleter()
{
    QStringList completionList;
    salesConnection->conn_open();
    if (salesConnection->conn_open()){
...
void SalesCenter::getRowToEdit()
{
    *rowToEditFromSelection = ui->tableWidget->currentRow();
    rowDefined = true;
}

void SalesCenter::reduceQuantityPurchased()
{
...
void SalesCenter::updateStockAndStockLogs(QString &currentBarCode, int &productPurchased, int &productAvailable)
{
...
void SalesCenter::disableSystems()
{
...
void SalesCenter::enableSystems()
{
...
void SalesCenter::checkLastSession()
{
    if(salesConnection->conn_open()){
        QSqlQuery query(QSqlDatabase::database("MyConnect"));
...
void SalesCenter::enableSystemsCalled()
{
...
void SalesCenter::receiveClosingComplete()
{
...
void SalesCenter::receiveOpeningComplete()
{
...
void SalesCenter::on_btnCompleteSales_clicked()
{
    goToPaymentAndCompleteSale();
}

void SalesCenter::addCompleterProductToSales(std::map<QString, int> &)
{

}

void SalesCenter::addProductToCart()
{

}

void SalesCenter::on_btn_addNewCustomer_clicked()
{
...
void SalesCenter::on_btn_viewCustomers_clicked()
{
...
void SalesCenter::receiveCustomerAdditionComplete()
{
    addNewCustomer->close();
    emit customerAdditionViaSalesClientComplete();
}

void SalesCenter::receiveCustomerSingleViewComplete()
{
    customerViewChoice->close();
    this->show();
}

void SalesCenter::on_btnReduceQtyByOne_2_clicked()
{
    reduceQuantityPurchased();
}

void SalesCenter::on_btnOpenClose_clicked()
{
    if(thereIsOpenSession){
        *executionType = "Closing";
        sessionControl = new SessionControl(this, *currentUser, *executionType);
...
void SalesCenter::on_btnReduceQtyByOne_clicked()
{
    reduceQuantityPurchased();
}
